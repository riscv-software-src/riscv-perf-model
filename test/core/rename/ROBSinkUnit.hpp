...
namespace rename_test
{
    ...
    class ROBSinkUnit : public core_test::SinkUnit
    {
        public:
            static constexpr char name[] = "ROBSinkUnit";

            ROBSinkUnit(sparta::TreeNode * n, const SinkUnitParameters * params) : SinkUnit(n, params)
            {
                if(params->purpose == "grp") {
                    in_sink_retire_inst_.registerConsumerHandler
                        (CREATE_SPARTA_HANDLER_WITH_DATA(ROBSinkUnit, sinkRetireInst_<olympia::InstGroupPtr>, olympia::InstGroupPtr));
                }
                else {
                    in_sink_retire_inst_.registerConsumerHandler
                        (CREATE_SPARTA_HANDLER_WITH_DATA(ROBSinkUnit, sinkRetireInst_<olympia::InstPtr>, olympia::InstPtr));
                }
            }
        private:
    ...
    using ROBSinkUnitFactory = sparta::ResourceFactory<ROBSinkUnit, ROBSinkUnit::SinkUnitParameters>;
}
