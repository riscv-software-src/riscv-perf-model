...
namespace olympia
{
    ...
    class Rename : public sparta::Unit
    {
      ...
      private:
        ...
        sparta::DataInPort<InstGroupPtr> in_uop_queue_append_{
            &unit_port_set_, "in_uop_queue_append", 1};
        sparta::DataOutPort<uint32_t> out_uop_queue_credits_{
            &unit_port_set_, "out_uop_queue_credits"};
        sparta::DataOutPort<InstGroupPtr> out_dispatch_queue_write_{
            &unit_port_set_, "out_dispatch_queue_write"};
        ...
        sparta::DataInPort<InstPtr> in_rename_retire_ack_{
            &unit_port_set_, "in_rename_retire_ack", 1};
        ...
        sparta::UniqueEvent<> ev_rename_insts_{
            &unit_event_set_, "rename_insts", CREATE_SPARTA_HANDLER(Rename, renameInstructions_)};
        sparta::UniqueEvent<> ev_schedule_rename_{
            &unit_event_set_, "schedule_rename", CREATE_SPARTA_HANDLER(Rename, scheduleRenaming_)};
        ...
        std::array<sparta::CycleCounter, N_STALL_REASONS> stall_counters_{
            {sparta::CycleCounter(
                 getStatisticSet(), "stall_no_decode_insts", "No Decode Insts",
                 sparta::Counter::COUNT_NORMAL, getClock()
             ),
             sparta::CycleCounter(
                 getStatisticSet(), "stall_no_dispatch_credits", "No Dispatch Credits",
                 sparta::Counter::COUNT_NORMAL, getClock()
             ),
             sparta::CycleCounter(
                 getStatisticSet(), "stall_no_renames", "No Renames", sparta::Counter::COUNT_NORMAL,
                 getClock()
             ),
             sparta::CycleCounter(
                 getStatisticSet(), "stall_not_stalled",
                 "Rename not stalled, all instructions renamed", sparta::Counter::COUNT_NORMAL,
                 getClock()
             )}};
