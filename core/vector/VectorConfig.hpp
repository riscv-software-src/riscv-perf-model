...
namespace olympia
{
    ...
    class VectorConfig
    {
      public:
            ...
            vta_(vta)
        {
            ...
            sparta_assert(lmul_ <= 8, "LMUL (" << lmul_ << ") cannot be greater than " << 8);
      ...
      private:
        ...
        uint32_t vlmax_formula_() { return (VLEN / sew_) * lmul_; }
    };
    ...
    {
        os << "e" << vector_config.getSEW() << "m" << vector_config.getLMUL()
           << (vector_config.getVTA() ? "ta" : "") << " vl: " << vector_config.getVL()
