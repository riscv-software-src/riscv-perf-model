...
template <
    typename InstType, typename AnnotationType, typename InstTypeAllocator,
    typename AnnotationTypeAllocator>
class Mavis;

namespace olympia
{
    using MavisType = Mavis<
        Inst, InstArchInfo, InstPtrAllocator<InstAllocator>,
      ...
      public:
        ...
        class MavisParameters : public sparta::ParameterSet
        {
          public:
            ...
            PARAMETER(
                std::string, isa_file_path, "mavis_isa_files", "Where are the mavis isa files?"
            )
            PARAMETER(
                std::string, uarch_file_path, "arches/isa_json", "Where are the mavis uarch files?"
            )
            PARAMETER(
                std::string, pseudo_file_path, "",
