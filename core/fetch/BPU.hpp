...
namespace BranchPredictor
{
    ...
    {
        public:
            PredictionRequest(uint64_t PC, uint8_t instType) :
                PC_(PC), instType_(instType) {}
        private:
            uint64_t PC_;
            uint8_t  instType_;
    };
    ...
    {
        public:
            PredictionOutput(bool predDirection, uint64_t predPC) : 
                predDirection_(predDirection), predPC_(predPC) {}
        private:
            bool predDirection_;
            uint64_t predPC_;
    };
    ...
    class UpdateInput
    {
        public:
            UpdateInput(uint64_t instrPC, bool correctedDirection, uint64_t correctedTargetPC) : 
                instrPC_(instrPC), correctedDirection_(correctedDirection), correctedTargetPC_(correctedTargetPC) {}
        private:
            uint64_t instrPC_;
    ...
    class BPU : public BranchPredictorIF<PredictionOutput, UpdateInput, PredictionRequest>, public sparta::Unit
    {
        public:
        class BPUParameterSet : public sparta::ParameterSet
        {
            public:
            BPUParameterSet(sparta::TreeNode* n) :
                sparta::ParameterSet(n) {}

                PARAMETER(uint32_t, ghr_size, 1024, "Number of history bits in GHR");
        };
        BPU(sparta::TreeNode * name,
              const BPUParameterSet * p);
        ...
        uint32_t predictionOutputCredit_ = 0;
        ...
        sparta::DataInPort<uint32_t> in_fetch_predictionOutput_credits_
            {&unit_port_set_, "in_fetch_predictionOutput_credits", sparta::SchedulingPhase::Tick, 0};
