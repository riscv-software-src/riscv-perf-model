...
public:
  DispatchSim(sparta::Scheduler *sched, const std::string &mavis_isa_files,
      ...
      : sparta::app::Simulation("DispatchSim", sched), input_file_(input_file),
...
private:
  void buildTree_() override {
    ...
    decode_unit->getParameterSet()
        ->getParameter("test_type")
        ->setValueFromString("multiple");
    // Create Dispatch
    tns_to_delete_.emplace_back(
    ...
    tns_to_delete_.emplace_back(
        rob = new sparta::ResourceTreeNode(
            ...
            sparta::TreeNode::GROUP_IDX_NONE, "Sink Unit", &rob_sink_fact));
