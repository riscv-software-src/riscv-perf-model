...
from database_explorer.sources.base import SourceHandler
from data.metadata import Metadata
from data.trace_data import TraceData

class DatabaseExplorer:
    def __init__(self, storage: SourceHandler):        
        self.source = storage
    
    def get_workload_id(self, workload_sha256: str) -> Optional[int]:
        workload_traces = self.source.traces_table[self.source.traces_table[TracesTableSchema.WORKLOAD_SHA] == workload_sha256]
    ...
    def upload_workload(self, workload_path: str) -> int:
        ...
        self.source.insert_workload(workload_path, workload_id)
        return workload_id

    def _get_next_workload_id(self) -> int:
        workload_ids = self.source.traces_table[TracesTableSchema.WORKLOAD_ID].unique()
    ...
    def get_workload_name(self, workload_id: int) -> Optional[str]:
        workload_row = self.source.workloads_table[self.source.workloads_table[WorkloadsTableSchema.WORKLOAD_ID] == workload_id]
    ...
    def get_trace_attempts(self, workload_id: int) -> list[int]:
        workload_traces = self.source.traces_table[self.source.traces_table[TracesTableSchema.WORKLOAD_ID] == workload_id]
    ...
    def get_trace_parts(self, workload_id: int, trace_attempt: int) -> list[int]:
        workload_traces = self.source.traces_table[
            (self.source.traces_table[TracesTableSchema.WORKLOAD_ID] == workload_id) &
            (self.source.traces_table[TracesTableSchema.TRACE_ATTEMPT] == trace_attempt)
        ]
        
        return workload_traces[TracesTableSchema.TRACE_PART].unique().tolist()
    
    def check_trace_exists(self, trace_id: str) -> bool:
        return trace_id in self.source.traces_table[TracesTableSchema.TRACE_ID].values

    def upload_traces(self, traces: list[TraceData]) -> None:
        self.source.update_traces_table()
        for trace in traces:        
    ...
    def _upload_trace(self, trace_path: str, metadata: Metadata) -> None:
        ...
        self.source.insert_trace(trace_path, metadata)

    def is_fully_traced(self, workload_id: int, trace_attempt: int) -> Optional[bool]:
        trace_row = self.source.traces_table[
            (self.source.traces_table[TracesTableSchema.WORKLOAD_ID] == workload_id) &
            (self.source.traces_table[TracesTableSchema.TRACE_ATTEMPT] == trace_attempt)
        ]
        if len(trace_row) == 0:
    ...
    def get_trace_ids(self) -> list[str]:
        return self.source.traces_table[TracesTableSchema.TRACE_ID].to_list()
    
    def get_metadata(self, trace_id: str) -> Metadata:
        return self.source.get_metadata(trace_id)
    
    def save_metadata(self, trace_id: str, path: str) -> None:
        return self.source.save_metadata(trace_id, path)
    
    def save_trace(self, trace_id: str, path: str) -> None:
        return self.source.save_trace(trace_id, path)
    
    def save_workload(self, workload_id: int, path: str) -> None:
        return self.source.save_workload(workload_id, path)