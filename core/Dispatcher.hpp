...
namespace olympia
{
    ...
    class Dispatcher
    {
      public:
        // Create a dispatcher
        Dispatcher(
            const std::string & name, Dispatch* dispatch, sparta::log::MessageSource & info_logger,
            sparta::DataInPort<uint32_t>* in_credits,
            sparta::DataOutPort<InstQueue::value_type>* out_inst
        )
            : name_(name), dispatch_(dispatch), info_logger_(info_logger), out_inst_(out_inst)
        {
            in_credits->registerConsumerHandler(
                CREATE_SPARTA_HANDLER_WITH_DATA(Dispatcher, receiveCredits_, uint32_t)
            );
        }
        ...
        void acceptInst(const InstPtr & inst)
        {
            sparta_assert(
                unit_credits_ != 0,
                "Dispatcher " << name_
                              << " cannot accept the given instruction (not enough credits): "
                              << inst
            );
            sparta_assert(
                num_can_dispatch_ != 0,
