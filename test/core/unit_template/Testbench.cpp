...
{
    using DutFactory = sparta::ResourceFactory<olympia::Dut, olympia::Dut::DutParameterSet>;
  ...
  private:
    ...
    void bindTree_() override
    {
        ...
        sparta::bind(root_node->getChildAs<sparta::Port>("dut.ports.i_instgrp_write"),
                     root_node->getChildAs<sparta::Port>("src.ports.o_instgrp_write"));

        // B - dut gives credits back to source
        sparta::bind(root_node->getChildAs<sparta::Port>("dut.ports.o_restore_credits"),
                     root_node->getChildAs<sparta::Port>("src.ports.i_credits"));

        // Cx - dut sends instgrp to sink
        sparta::bind(root_node->getChildAs<sparta::Port>("dut.ports.o_instgrp_write"),
                     root_node->getChildAs<sparta::Port>("sink.ports.i_instgrp_write"));

        // Dx - sink gives credits back to dut
        sparta::bind(root_node->getChildAs<sparta::Port>("dut.ports.i_credits"),
                     root_node->getChildAs<sparta::Port>("sink.ports.o_restore_credits"));
    }
