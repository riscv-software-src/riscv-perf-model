...
namespace olympia {
  ...
  public:
     ...
     *
     */
    virtual void updatePrefetcherState(const PrefetcherStateUpdateType & prefetcher_state) { (void) prefetcher_state; };
     ...
     *
     * \note having both getPrefetchMemoryAccess() and popPrefetchMemoryAccess() allows for implementers
     * to have the choice to use a seperate getPrefetchMemoryAccess()/popPrefetchMemoryAccess() or
     * combine both operations in getPrefetchMemoryAccess() and do nothing in popPrefetchMemoryAccess()
     */
