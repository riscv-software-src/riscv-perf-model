...
namespace olympia
{
    ...
    Dispatch::Dispatch(sparta::TreeNode* node, const DispatchParameterSet* p)
        : sparta::Unit(node),
          dispatch_queue_(
              "dispatch_queue", p->dispatch_queue_depth, node->getClock(), getStatisticSet()
          ),
          num_to_dispatch_(p->num_to_dispatch)
    {
        ...
        for (auto exe_unit_pair : execution_topology)
        {
            ...
            for (uint32_t unit_num = 0; unit_num < exe_idx; ++unit_num)
            {
                ...
                sparta_assert(
                    ...
                    "Unknown target unit: " << tgt_name
                                            << " when parsing the execution_topology extension"
                );
                dispatchers_[target_itr->second].emplace_back(new Dispatcher(
                    unit_name, this, info_logger_, in_credit_port.get(), out_inst_port.get()
                ));
            }
        }
        ...
        in_reorder_flush_.registerConsumerHandler(
            CREATE_SPARTA_HANDLER_WITH_DATA(Dispatch, handleFlush_, FlushManager::FlushingCriteria)
        );
    ...
    void Dispatch::dispatchInstructions_()
    {
        ...
        for (uint32_t i = 0; (i < num_dispatch) && keep_dispatching; ++i)
        {
            ...
            sparta_assert(
                target_unit != InstArchInfo::TargetUnit::UNKNOWN,
