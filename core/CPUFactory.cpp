 ...
 */
auto olympia::CPUFactory::buildTree_(
    sparta::RootTreeNode* root_node, const std::vector<olympia::CPUTopology::UnitInfo> & units
) -> void
{
    ...
    for (std::size_t num_of_cores = 0; num_of_cores < topology_->num_cores; ++num_of_cores)
    {
        for (const auto & unit : units)
        {
            ...
            std::replace(
                parent_name.begin(), parent_name.end(), to_replace_, *replace_with.c_str()
            );
            ...
            auto rtn = new sparta::ResourceTreeNode(
                parent_node, node_name, unit.group_name, unit.group_id, human_name, unit.factory
            );
            ...
            rtn->addExtensionFactory(
                olympia::CoreExtensions::name,
...
auto olympia::CPUFactory::bindTree_(
    ...
    const std::vector<olympia::CPUTopology::PortConnectionInfo> & ports
) -> void
{
    ...
    for (std::size_t num_of_cores = 0; num_of_cores < topology_->num_cores; ++num_of_cores)
    {
        for (const auto & port : ports)
        {
            ...
            replace_with = std::to_string(num_of_cores);
            std::replace(
                out_port_name.begin(), out_port_name.end(), to_replace_, *replace_with.c_str()
            );
            std::replace(
                in_port_name.begin(), in_port_name.end(), to_replace_, *replace_with.c_str()
            );
            sparta::bind(
