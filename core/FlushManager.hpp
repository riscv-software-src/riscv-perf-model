    ...
    class FlushManager : public sparta::Unit
    {
      public:
        ...
        FlushManager(sparta::TreeNode* rc, const FlushManagerParameters* params)
            : Unit(rc, name), out_retire_flush_(getPortSet(), "out_retire_flush", false),
        ...
        {
            (void)params;
            in_retire_flush_.registerConsumerHandler(
                CREATE_SPARTA_HANDLER_WITH_DATA(FlushManager, forwardRetireFlush_, FlushingCriteria)
            );
            in_fetch_flush_redirect_.registerConsumerHandler(
                CREATE_SPARTA_HANDLER_WITH_DATA(FlushManager, forwardFetchRedirectFlush_, uint64_t)
            );
        }
