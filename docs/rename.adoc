...
PRFs are reclaimed and pushed into the freelist in one of two ways:

- Instructions that retire are sent back to Rename to reclaim the old
  PRF that the instruction overwrote (see `old_prf` above).
...
multiple ARFs to the same PRF.  For example:
...
operations and send the instruction to the middle machine to "execute."
...
This is inefficient as two PRFs are now consumed representing the same contents as `x1 -> x55`.
