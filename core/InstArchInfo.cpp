...
namespace olympia
{
    ...
    void InstArchInfo::update(const nlohmann::json & jobj)
    {
        ...
        if (jobj.find("dispatch") != jobj.end())
        {
            ...
            sparta_assert(
                itr != dispatch_target_map.end(),
                "Unknown target unit: " << tgt
                                        << " for inst: " << jobj["mnemonic"].get<std::string>()
            );
            tgt_unit_ = itr->second;
        }
        sparta_assert(
            tgt_unit_ != TargetUnit::UNKNOWN,
        ...
        if (jobj.find("pipe") != jobj.end())
        {
            ...
            sparta_assert(
                itr != execution_pipe_map.end(),
                "Unknown pipe target: " << pipe_name
                                        << " for inst: " << jobj["mnemonic"].get<std::string>()
            );
            tgt_pipe_ = itr->second;
        }
        sparta_assert(
            tgt_pipe_ != TargetPipe::UNKNOWN,
