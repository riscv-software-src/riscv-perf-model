...
void olympia::CoreTopologySimple::bindTree(sparta::RootTreeNode* root_node)
{
    ...
    for (uint32_t core_num = 0; core_num < num_cores; ++core_num)
    {
        ...
        const auto issue_queue_topology = olympia::coreutils::getPipeTopology(
            root_node->getChild(core_node), "issue_queue_topology");
        for (size_t i = 0; i < issue_queue_topology.size(); ++i)
        {
            ...
            for (auto exe_unit : issue_queue_topology[i])
            {
                const std::string exe_pipe_out =
                    core_node + ".execute." + exe_unit + ".ports.out_execute_pipe";
