...
namespace olympia
{
    ...
    class DataPrefetcher : public sparta::Unit, public PrefetcherIF<PrefetchEngineIF<>>
    {
      public:
        ...
        {
          public:
            DataPrefetcherParameterSet(sparta::TreeNode* n) : sparta::ParameterSet(n) {}

            PARAMETER(std::string, prefetcher_type, "next_line", "next_line");
            PARAMETER(uint32_t, num_to_prefetch, 1,
                      "Number of cache lines to prefetch per incoming memory access");
         ...
         */
        DataPrefetcher(sparta::TreeNode* node, const DataPrefetcherParameterSet* p);
