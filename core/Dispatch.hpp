...
namespace olympia
{
    ...
    class Dispatch : public sparta::Unit
    {
      public:
        ...
        class DispatchParameterSet : public sparta::ParameterSet
        {
          public:
            ...
            PARAMETER(
                std::vector<double>, context_weights, std::vector<double>(1, 1),
      ...
      private:
        ...
        // Ports
        sparta::DataInPort<InstGroupPtr> in_dispatch_queue_write_{
            &unit_port_set_, "in_dispatch_queue_write", 1};
        sparta::DataOutPort<uint32_t> out_dispatch_queue_credits_{
            &unit_port_set_, "out_dispatch_queue_credits"};
        ...
        sparta::DataInPort<uint32_t> in_lsu_credits_{
            &unit_port_set_, "in_lsu_credits", sparta::SchedulingPhase::Tick, 0};
        sparta::DataOutPort<InstQueue::value_type> out_lsu_write_{
            &unit_port_set_, "out_lsu_write", false};
        sparta::DataInPort<uint32_t> in_reorder_credits_{
            &unit_port_set_, "in_reorder_buffer_credits", sparta::SchedulingPhase::Tick, 0};
        sparta::DataOutPort<InstGroupPtr> out_reorder_write_{
            &unit_port_set_, "out_reorder_buffer_write"};
        ...
        std::array<sparta::CycleCounter, N_STALL_REASONS> stall_counters_{
            {sparta::CycleCounter(
                 getStatisticSet(), "stall_alu_busy", "ALU busy", sparta::Counter::COUNT_NORMAL,
                 getClock()
             ),
             sparta::CycleCounter(
                 getStatisticSet(), "stall_fpu_busy", "FPU busy", sparta::Counter::COUNT_NORMAL,
                 getClock()
             ),
             sparta::CycleCounter(
                 getStatisticSet(), "stall_br_busy", "BR busy", sparta::Counter::COUNT_NORMAL,
                 getClock()
             ),
             sparta::CycleCounter(
                 getStatisticSet(), "stall_lsu_busy", "LSU busy", sparta::Counter::COUNT_NORMAL,
                 getClock()
             ),
             sparta::CycleCounter(
                 getStatisticSet(), "stall_no_rob_credits", "No credits from ROB",
                 sparta::Counter::COUNT_NORMAL, getClock()
             ),
             sparta::CycleCounter(
                 getStatisticSet(), "stall_not_stalled",
                 "Dispatch not stalled, all instructions dispatched", sparta::Counter::COUNT_NORMAL,
                 getClock()
             )}};

        std::array<sparta::Counter, InstArchInfo::N_TARGET_UNITS> unit_distribution_{
            {sparta::Counter(
                 getStatisticSet(), "count_alu_insts", "Total ALU insts",
                 sparta::Counter::COUNT_NORMAL
             ),
             sparta::Counter(
                 getStatisticSet(), "count_fpu_insts", "Total FPU insts",
                 sparta::Counter::COUNT_NORMAL
             ),
             sparta::Counter(
                 getStatisticSet(), "count_br_insts", "Total BR insts",
                 sparta::Counter::COUNT_NORMAL
             ),
             sparta::Counter(
                 getStatisticSet(), "count_lsu_insts", "Total LSU insts",
                 sparta::Counter::COUNT_NORMAL
             ),
             sparta::Counter(
                 getStatisticSet(), "count_rob_insts", "Total ROB insts",
                 sparta::Counter::COUNT_NORMAL
             )}};
