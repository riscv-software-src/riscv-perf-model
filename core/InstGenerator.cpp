...
namespace olympia
{
    std::unique_ptr<InstGenerator> InstGenerator::createGenerator(
    ...
    )
    {
        ...
        // Dunno what it is...
        sparta_assert(
            false, "Unknown file extension for '" << filename << "'.  Expected .json or .[z]stf"
        );
        return nullptr;
    }
    ...
    InstPtr JSONInstGenerator::getNextInst(const sparta::Clock* clk)
    {
        ...
        auto addElement = [&jinst](
                              mavis::OperandInfo & operands, const std::string & key,
        ...
        addElement(
            srcs, "rs1", mavis::InstMetaData::OperandFieldID::RS1,
            mavis::InstMetaData::OperandTypes::LONG
        );
        addElement(
            srcs, "fs1", mavis::InstMetaData::OperandFieldID::RS1,
            mavis::InstMetaData::OperandTypes::DOUBLE
        );
        addElement(
            srcs, "rs2", mavis::InstMetaData::OperandFieldID::RS2,
            mavis::InstMetaData::OperandTypes::LONG
        );
        addElement(
            srcs, "fs2", mavis::InstMetaData::OperandFieldID::RS2,
            mavis::InstMetaData::OperandTypes::DOUBLE
        );

        mavis::OperandInfo dests;
        addElement(
            dests, "rd", mavis::InstMetaData::OperandFieldID::RD,
            mavis::InstMetaData::OperandTypes::LONG
        );
        addElement(
            dests, "fd", mavis::InstMetaData::OperandFieldID::RD,
            mavis::InstMetaData::OperandTypes::DOUBLE
        );
    ...
    ////////////////////////////////////////////////////////////////////////////////
    // STF Inst Generator
    TraceInstGenerator::TraceInstGenerator(
        MavisType* mavis_facade, const std::string & filename, const bool skip_nonuser_mode
    )
        : InstGenerator(mavis_facade)
    {
        ...
        reader_.reset(new stf::STFInstReader(
            filename, skip_nonuser_mode, CHECK_FOR_STF_PTE, FILTER_MODE_CHANGE_EVENTS, BUFFER_SIZE
        ));
    ...
    InstPtr TraceInstGenerator::getNextInst(const sparta::Clock* clk)
    {
        ...
        try
        {
            ...
            if (const auto & mem_accesses = next_it_->getMemoryAccesses(); !mem_accesses.empty())
            {
                ...
                std::for_each(
                    next_it_->getMemoryAccesses().begin(), next_it_->getMemoryAccesses().end(),
