...
namespace olympia
{
    ...
    bool StridePrefetchEngine::handleMemoryAccess(const MemoryAccessInfoPtr & access)
    {
        ...
        sparta::memory::addr_t cache_line_addr = getCacheLineAddr(addr);
        ...
        // Evict lowest-confidence entry if table is full and this is a new index
        if (stride_table_.find(index) == stride_table_.end()
            && stride_table_.size() >= table_size_)
        {
            // Find entry with lowest confidence to evict
            auto victim = stride_table_.begin();
            for (auto it = stride_table_.begin(); it != stride_table_.end(); ++it)
            {
                if (it->second.confidence < victim->second.confidence)
                {
                    victim = it;
                }
            }
            stride_table_.erase(victim);
        }
