...
namespace olympia
{
    ...
    ExecutePipe::ExecutePipe(sparta::TreeNode* node, const ExecutePipeParameterSet* p)
        : sparta::Unit(node), ignore_inst_execute_time_(p->ignore_inst_execute_time),
          execute_time_(p->execute_time), scheduler_size_(p->scheduler_size),
          in_order_issue_(p->in_order_issue), reg_file_(determineRegisterFile(node->getGroup())),
          collected_inst_(node, node->getName())
    {
        ...
        in_reorder_flush_.registerConsumerHandler(
            CREATE_SPARTA_HANDLER_WITH_DATA(ExecutePipe, flushInst_, FlushManager::FlushingCriteria)
        );
    ...
    void ExecutePipe::issueInst_()
    {
        ...
        sparta_assert_context(
            unit_busy_ == false && ready_queue_.size() > 0,
    ...
    void ExecutePipe::completeInst_(const InstPtr & ex_inst)
    {
        ...
        if (SPARTA_EXPECT_FALSE(ex_inst->isTransfer()))
        {
            if (ex_inst->getPipe() == InstArchInfo::TargetPipe::I2F)
            {
                ...
                sparta_assert(
                    reg_file_ == core_types::RegFile::RF_INTEGER,
                    "Got an I2F instruction in an ExecutionPipe that does not source the integer "
                    "RF: "
                        << ex_inst
                );
            ...
            else
            {
                ...
                sparta_assert(
                    ex_inst->getPipe() == InstArchInfo::TargetPipe::F2I,
