...
namespace olympia
{
    ...
    std::vector<std::string> getISAFiles(
        sparta::TreeNode* n, const std::string & isa_file_path, const std::string & pseudo_file_path
    )
    {
    ...
    std::vector<std::string> getUArchFiles(
        sparta::TreeNode* n, const MavisUnit::MavisParameters* p,
        const std::string & uarch_file_path, const std::string & pseudo_file_path
    )
    {
        std::vector<std::string> uarch_files
            = {uarch_file_path + "/olympia_uarch_rv64g.json",
    ...
    MavisUnit::MavisUnit(sparta::TreeNode* n, const MavisParameters* p)
        : sparta::Unit(n),
          pseudo_file_path_(
              std::string(p->pseudo_file_path).empty() ? p->uarch_file_path : p->pseudo_file_path
          ),
