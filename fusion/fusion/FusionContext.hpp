...
namespace fusion
{
    ...
    // -----------------------------------------------------------------------
    template <typename FusionGroupType> class FusionContext
    {
      ...
      public:
        ...
        void makeContext(const std::string & n, const FusionGroupListType & groups)
        {
            name_ = n;
        ...
        void insertGroup(const FusionGroupType & group)
        {
            ...
            if (hash == 0)
            {
                throw fusion::HashIllegalValueError(group.name(), group.hash());
            }
