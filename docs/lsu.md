# LSU
The design of the LSU is losely based on the LSU implementation in [BOOM](https://docs.boom-core.org/en/latest/sections/load-store-unit.html) microarchitecture and the [E500](https://www.nxp.com/docs/en/reference-manual/E500CORERM.pdf) core.

### Ports
in_lsu_insts <--> dispatch (Receive new memory instructions)

in_cache_lookup_ack <--> DCache (Receive acknowledgement from cache if the data is present)

in_cache_lookup_req <--> DCache (Receive data for the cache lookup request)

in_cache_free_req <--> DCache (Cache lets the lsu know that its ready to accept new lookup requests)

out_cache_lookup_req <--> DCache (Send a cache lookup request for a particular address)

in_mmu_lookup_ack <--> MMU (Receive acknowledgement from MMU  of particular virtual address lookup)

in_mmu_lookup_req <--> MMU (Receive physical address for the virtual address lookup request)

out_mmu_lookup_req <--> DCache (Send a VA to PA address translation)
