...
public:
    ...
    public:
        ...
        PARAMETER(uint32_t, fusion_enable_register, (uint32_t)-1,
        ...
        PARAMETER(uint32_t,fusion_match_max_tries,1023,
                           "fusion group match attempts limit")
        ...
        //! \brief ...
        PARAMETER(std::string, fusion_summary_report,
                  "fusion_summary.txt",
...
private:
    ...
    sparta::UniqueEvent<> ev_decode_insts_event_  {
        &unit_event_set_, "decode_insts_event",
        CREATE_SPARTA_HANDLER(Decode, decodeInsts_)};

    //! \brief initialize the fusion group configuration
    void constructFusionGroups();
    ...
    void matchFusionGroups(MatchInfoListType& matches,InstGroupPtr &insts,
                     InstUidListType& inputUIDS, FusionGroupContainerType &);

    //! \brief process the fusion matches
    void processMatches(MatchInfoListType &,InstGroupPtr &insts,
    ...
    void whereIsEgon(InstGroupPtr&,uint32_t&);

    //! \brief ...
    void infoInsts(std::ostream &os,const InstGroupPtr &insts);
    ...
    void setFusionContext(FusionGroupListType&);

    //! \brief fusion groups are contructed statically or from DSL files
    void constructGroups(const FusionGroupCfgListType&,FusionGroupListType&);

    //! \brief record stats for fusionGroup name
    void updateFusionGroupUtilization(std::string name);
    ...
    //! counts only once.
    sparta::Counter fusion_num_groups_utilized;
    ...
    //!
    //! \see Fusion
    FusionType *fuser{nullptr};
    fusion::HCache hcache;

    //! \brief fusion function object callback proxies
    struct cbProxy;
   //! \brief the set of implemented fusable sequences 
   static FusionType::FusionGroupCfgListType fusionGroupCfgs;
   //! \brief this counts the number of times a group is used
    std::map<std::string,uint32_t> matchedFusionGroups;
