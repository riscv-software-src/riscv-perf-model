...
class DatabaseExplorer:
    ...
    def get_workload_id(self, workload_sha256: str) -> Optional[int]:
        workload_traces = self.storage.traces_table[self.storage.traces_table[TracesTableSchema.WORKLOAD_SHA] == workload_sha256]
    ...
    def get_workload_name(self, workload_id: int) -> Optional[str]:
        workload_row = self.storage.workloads_table[self.storage.workloads_table[WorkloadsTableSchema.WORKLOAD_ID] == workload_id]
    ...
    def get_trace_attempts(self, workload_id: int) -> list[int]:
        workload_traces = self.storage.traces_table[self.storage.traces_table[TracesTableSchema.WORKLOAD_ID] == workload_id]
    ...
    def get_trace_parts(self, workload_id: int, trace_attempt: int) -> list[int]:
        workload_traces = self.storage.traces_table[
            ...
            (self.storage.traces_table[TracesTableSchema.TRACE_ATTEMPT] == trace_attempt)
        ]
