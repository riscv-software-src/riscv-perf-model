...
namespace olympia
{
    class SimpleCacheLine : public sparta::cache::BasicCacheItem
    {
      public:
        ...
        SimpleCacheLine(uint64_t line_size) : line_size_(line_size), valid_(false)
        {
            sparta_assert(
                sparta::utils::is_power_of_2(line_size),
        ...
        SimpleCacheLine(const SimpleCacheLine & rhs)
            : BasicCacheItem(rhs), line_size_(rhs.line_size_), valid_(rhs.valid_)
        {
        }
...

    {
      public:
        ...
        SimpleDL1(
            sparta::TreeNode* parent, uint64_t cache_size_kb, uint64_t line_size,
            const sparta::cache::ReplacementIF & rep
        )
            : sparta::cache::SimpleCache2<SimpleCacheLine>(
                cache_size_kb, line_size, line_size, SimpleCacheLine(line_size), rep
            ),
              sparta::TreeNode(parent, "l1cache", "Simple L1 DCache"),
              sparta::cache::PreloadableIF(), sparta::cache::PreloadDumpableIF(),
              preloadable_(
                  this, std::bind(&SimpleDL1::preloadPkt_, this, _1),
      ...
      private:
        ...
        bool preloadPkt_(sparta::cache::PreloadPkt & pkt) override
        {
            ...
            for (auto & line_data : lines)
            {
                ...
                std::cout << *this << " : Preloading VA: 0x" << std::hex << va << std::endl;
        ...
        void preloadDump_(sparta::cache::PreloadEmitter & emitter) const override
        {
            ...
            for (auto set_it = begin(); set_it != end(); ++set_it)
            {
                for (auto line_it = set_it->begin(); line_it != set_it->end(); ++line_it)
                {
                    if (line_it->isValid())
                    {
