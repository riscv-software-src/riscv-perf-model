...
namespace olympia
{
    ...
    class ROB : public sparta::Unit
    {
      public:
        ...
        {
          public:
            ROBParameterSet(sparta::TreeNode* n) : sparta::ParameterSet(n) {}
            ...
            PARAMETER(
                uint32_t, num_insts_to_retire, 0,
            ...
            PARAMETER(
                sparta::Clock::Cycle, retire_timeout_interval, 10000,
                "Retire timeout error threshold (in cycles). Amount of time elapsed when nothing "
                "was retired"
            )
        };
      ...
      private:
        ...
        sparta::StatisticInstance
            overall_ipc_si_; // An overall IPC statistic instance starting at time == 0
        sparta::StatisticInstance
            period_ipc_si_; // An IPC counter for the period between retirement heartbeats
        ...
        sparta::DataInPort<InstGroupPtr> in_reorder_buffer_write_{
            &unit_port_set_, "in_reorder_buffer_write", 1};
        sparta::DataOutPort<uint32_t> out_reorder_buffer_credits_{
            &unit_port_set_, "out_reorder_buffer_credits"};
        sparta::DataInPort<bool> in_oldest_completed_{
            &unit_port_set_, "in_reorder_oldest_completed"};
        sparta::DataOutPort<FlushManager::FlushingCriteria> out_retire_flush_{
            &unit_port_set_, "out_retire_flush"};
        sparta::DataOutPort<uint64_t> out_fetch_flush_redirect_{
            &unit_port_set_, "out_fetch_flush_redirect"};

        // UPDATE:
        sparta::DataOutPort<InstPtr> out_rob_retire_ack_{&unit_port_set_, "out_rob_retire_ack"};
        sparta::DataOutPort<InstPtr> out_rob_retire_ack_rename_{
            &unit_port_set_, "out_rob_retire_ack_rename"};
        ...
        sparta::UniqueEvent<> ev_retire_{
            &unit_event_set_, "retire_insts", CREATE_SPARTA_HANDLER(ROB, retireEvent_)};
        ...
        sparta::Clock::Cycle last_retirement_
            = 0; // Last retirement cycle for checking stalled retire
        sparta::Event<> ev_ensure_forward_progress_{
            &unit_event_set_, "forward_progress_check",
