...
namespace olympia
{
   ...
   *
   */
  template<typename PrefetchEngineIF, typename PrefetcherStateUpdateType = NullPrefetcherStateUpdateType>
  class PrefetcherIF {
  public:
    /**
    ...
    PrefetcherIF(sparta::Unit *unit):
      ...
      in_reorder_flush_{unit_->getPortSet(), "in_reorder_flush", sparta::SchedulingPhase::Flush, 1},
    ...
    {
      in_req_.
        registerConsumerHandler(CREATE_SPARTA_HANDLER_WITH_DATA(PrefetcherIF, processIncomingReq_, MemoryAccessInfoPtr));
      in_reorder_flush_.
            registerConsumerHandler(CREATE_SPARTA_HANDLER_WITH_DATA(PrefetcherIF, handleFlush_, FlushManager::FlushingCriteria));
    }
    ...
    */
    void setEngine(std::unique_ptr<PrefetchEngineIF> engine) {
    ...
    */
    virtual bool isPrefetchReady() {
      return engine_->isPrefetchReady();
    }
    ...
    */
     ...
     */
    std::unique_ptr<PrefetchEngineIF> &getPrefetchEngine() {
      return engine_;
    }
   ...
   private:
    ...
    void processIncomingReq_(const MemoryAccessInfoPtr& access){
      processIncomingReq(access);
    }
