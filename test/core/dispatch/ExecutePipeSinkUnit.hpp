...
namespace core_test
{
    ...
    class ExecutePipeSinkUnit : public sparta::Unit
    {
    public:
        static constexpr char name[] = "ExecutePipeSinkUnit";

        class ExecutePipeSinkUnitParameters : public sparta::ParameterSet
        {
        public:
            explicit ExecutePipeSinkUnitParameters(sparta::TreeNode *n) :
                sparta::ParameterSet(n)
            { }
            PARAMETER(uint32_t, sink_queue_size, 10, "Sink queue size for testing")
            PARAMETER(std::string, purpose, "grp", "Purpose of this ExecutePipeSinkUnit: grp, single")
        };

        ExecutePipeSinkUnit(sparta::TreeNode * n, const ExecutePipeSinkUnitParameters * params) :
            sparta::Unit(n),
            ...
            credits_to_send_back_(credits_)
        {
            if(params->purpose == "grp") {
                in_sink_inst_.registerConsumerHandler
                    (CREATE_SPARTA_HANDLER_WITH_DATA(ExecutePipeSinkUnit, sinkInst_<olympia::InstGroupPtr>, olympia::InstGroupPtr));
            }
            else {
                in_sink_inst_.registerConsumerHandler
                    (CREATE_SPARTA_HANDLER_WITH_DATA(ExecutePipeSinkUnit, sinkInst_<olympia::InstPtr>, olympia::InstPtr));
            }
            sparta::StartupEvent(n, CREATE_SPARTA_HANDLER(ExecutePipeSinkUnit, sendCredits_));
        }
    ...
    private:
        ...
        sparta::UniqueEvent<> ev_return_credits_{&unit_event_set_, "return_credits",
                                                 CREATE_SPARTA_HANDLER(ExecutePipeSinkUnit, sendCredits_)};
        uint32_t credits_to_send_back_ = 0;
    };

    using ExecutePipeSinkUnitFactory = sparta::ResourceFactory<ExecutePipeSinkUnit, ExecutePipeSinkUnit::ExecutePipeSinkUnitParameters>;
}
