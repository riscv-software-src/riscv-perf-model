...
template <typename AnnotationType> class Instruction
{
  ...
  public:
    ...
    bool isExtInstType(mavis::DecodedInstructionInfo::ExtractedInstTypes itype) const
    {
    ...
    const mavis::OperandInfo & getSourceOpInfo() const { return dinfo_->getSourceOpInfo(); }

    const mavis::OperandInfo & getDestOpInfo() const { return dinfo_->getDestOpInfo(); }

    mavis::InstructionUniqueID getUID() const { return dinfo_->getInstructionUniqueID(); }
    ...
    void print(std::ostream & os) const
    {
        ...
        os << dinfo_->getMnemonic() << ", src[" << dinfo_->numSourceRegs()
           << "]: " << dinfo_->getSourceRegs() << " (addr: " << dinfo_->getSourceAddressRegs()
           << ")"
  ...
  public:
    friend std::ostream & operator<<(std::ostream & os, const Instruction<AnnotationType> & i)
    {
        i.print(os);
        return os;
    }
