...
namespace olympia
{
    ...
    NextLinePrefetchEngine::NextLinePrefetchEngine(unsigned int num_lines_to_prefetch,
    ...
    bool NextLinePrefetchEngine::isPrefetchReady() const { return !prefetch_queue_.empty(); }
    ...
    bool NextLinePrefetchEngine::handleMemoryAccess(const MemoryAccessInfoPtr & access)
    {
        ...
        for (uint64_t i = 0; i < num_lines_to_prefetch_; i++)
        {
            ...
            // FIX: Create a NEW MemoryAccessInfo object for the prefetch
            // instead of copying the shared pointer (which points to the old object).
            MemoryAccessInfoPtr prefetch_access = 
                sparta::SpartaSharedPointer<olympia::MemoryAccessInfo>(
                    new olympia::MemoryAccessInfo(addr)
                );

            // NOTE: Since we created a fresh MemoryAccessInfo from an address, 
            // it might not have an Instruction pointer attached (depending on Olympia's internals).
            // If you need to set metadata, do it directly on the MemoryAccessInfo if possible.
            
            // Only set InstPtr data if InstPtr exists to avoid SegFaults
