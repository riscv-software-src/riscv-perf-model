...
namespace olympia
{
    ...
    {
        // Register handler for credit port from DCache
        in_prefetcher_credits_.registerConsumerHandler(
            CREATE_SPARTA_HANDLER_WITH_DATA(Prefetcher, receiveCredits_, uint32_t));
    ...
    //! Receive credits from DCache (port-based, replaces restorePrefetchCredit)
    void Prefetcher::receiveCredits_(const uint32_t & credits)
    {
        sparta_assert((prefetcher_credits_ + credits) <= max_prefetcher_credits_,
                      "Attempted to restore prefetch credits beyond max");
        prefetcher_credits_ += credits;

        ILOG("Received " << credits << " prefetch credits, total: " << prefetcher_credits_);
