...
namespace olympia
{
    ...
    Rename::Rename(sparta::TreeNode* node, const RenameParameterSet* p)
        : sparta::Unit(node),
          uop_queue_(
              "rename_uop_queue", p->rename_queue_depth, node->getClock(), getStatisticSet()
          ),
          num_to_rename_per_cycle_(p->num_to_rename),
          rename_histogram_(
              *getStatisticSet(), "rename_histogram", "Rename Stage Histogram",
              [&p]()
              {
          ...
          )
    {
        ...
        auto setup_map = [this](core_types::RegFile reg_file, const uint32_t num_renames)
        {
            ...
            sparta_assert(
                num_regs < num_renames
            ); // ensure we have more renames than 32 because first 32 renames are allocated at the
               // beginning
        ...
        static_assert(
            core_types::RegFile::N_REGFILES == 2, "New RF type added, but Rename not updated"
        );
    }
    ...
    void Rename::getAckFromROB_(const InstPtr & inst_ptr)
    {
        ...
        if (dests.size() > 0)
        {
            ...
            // mapping for the ARF -> PRF so we know it's free to be pushed to freelist if it has no
        ...
        if (num_to_rename_ > 0)
        {
            ...
            for (uint32_t i = 0; i < num_to_rename_; ++i)
            {
                ...
                for (const auto & src : srcs)
                {
                    ...
                    if (renaming_inst->isLoadStoreInst())
                    {
                        ...
                        if (src.field_id == mavis::InstMetaData::OperandFieldID::RS2)
                        {
                            ...
                            ILOG(
                                "\tsetup store data register bit mask "
                        ...
                        else
                        {
                            ...
                            ILOG(
                                "\tsetup source register bit mask "
                                << sparta::printBitSet(bitmask) << " for '" << rf << "' scoreboard"
                            );
                        }
                    }
                    else
                    {
                        ...
                        ILOG(
                            "\tsetup source register bit mask " << sparta::printBitSet(bitmask)
                                                                << " for '" << rf << "' scoreboard"
                        );
                    }
                }
                ...
                for (const auto & dest : dests)
                {
                    ...
                    // "valid" PRF for that ARF anymore and there are no sources referring to it so
                    // we can push it to freelist
                    reference_counter_[rf][prf]++;
                    bitmask.set(prf);
                    // clear scoreboard for the PRF we are allocating
                    scoreboards_[rf]->clearBits(bitmask);
                    ILOG(
                        "\tsetup destination register bit mask " << sparta::printBitSet(bitmask)
                                                                 << " for '" << rf << "' scoreboard"
                    );
                }
