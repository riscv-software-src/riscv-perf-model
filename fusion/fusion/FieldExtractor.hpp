...
class FieldExtractor
{
  public:
    ...
    //! \brief type for getXRegs operations from mavis
    using RegsGetter = MavisBitMaskType (Instruction<uArchInfo>::*)() const;
    ...
    uint32_t getField(InstPtrType inst, FieldName field) const
    {
        ...
        {
            throw fusion::FieldExtUnknownField((uint32_t)field, inst->dasmString());
        }
    ...
    uint32_t getSField(InstPtrType inst, SFieldName field) const
    {
        ...
        if (value == 0)
        {
            throw fusion::FieldExtUnknownSpecialField((uint32_t)field, inst->dasmString());
        }
        return value;
    ...
    bool checkInstHasField(InstPtrType inst, FieldName field, bool & isDest) const
    {
        ...
        if (inst->getSourceOpInfo().hasFieldID(field) && field != FieldName::RS_MAX)
        {
            return true;
        }
    ...
    //! \brief equality
    bool eq(const InstPtrListType & input, size_t a, size_t b, const FieldName f1,
            const OptArg f2 = std::nullopt) const
    {
        return compare(input[a], input[b], f1, f2, EQ);
    }

    //! \brief less than
    bool lt(const InstPtrListType & input, size_t a, size_t b, const FieldName f1,
            const OptArg f2 = std::nullopt) const
    {
    ...
    //! \brief not equal
    bool noteq(const InstPtrListType & input, size_t a, size_t b, const FieldName f1,
               const OptArg f2 = std::nullopt) const
    {
        return !compare(input[a], input[b], f1, f2, EQ);
    }
    ...
    //! \brief greater than
    bool gt(const InstPtrListType & input, size_t a, size_t b, const FieldName f1,
            const OptArg f2 = std::nullopt) const
    {
    ...
    bool lteq(const InstPtrListType & input, size_t a, size_t b, const FieldName f1,
              const OptArg f2 = std::nullopt) const
    {
        return !compare(input[b], input[a], f1, f2, LT);
    }
    ...
    bool gteq(const InstPtrListType & input, size_t a, size_t b, const FieldName f1,
              const OptArg f2 = std::nullopt) const
    {
        return !compare(input[a], input[b], f1, f2, LT);
    }
    ...
    {
        return countPorts(input, &Instruction<uArchInfo>::getIntSourceRegs);
    }
    ...
    {
        return countPorts(input, &Instruction<uArchInfo>::getFloatSourceRegs);
    }
    ...
    {
        return countPorts(input, &Instruction<uArchInfo>::getFloatDestRegs);
    }
    ...
    {
        return countPorts(input, &Instruction<uArchInfo>::getVectorSourceRegs);
    }
    ...
    {
        return countPorts(input, &Instruction<uArchInfo>::getVectorDestRegs);
    }

  private:
    //! \brief compare common method
    bool compare(const InstPtrType LHS, const InstPtrType RHS, const FieldName f1, const OptArg _f2,
                 FUNC func) const
    {
        ...
        switch (func)
        {
            ...
            default:
            {
                throw fusion::FieldExtUnknownFunction((uint32_t)func);
            }
        }
        // clang-format on
    ...
    //! group
    uint32_t countPorts(const InstPtrListType & input, RegsGetter getRegs) const
    {
